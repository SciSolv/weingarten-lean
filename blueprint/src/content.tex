...
no irreducible-character theory --- including the exact vanishing of the signed sum
...
of known mathematics (the five prose remarks carry their own status flags, one
of them an explicitly marked conjecture); the paper proofs are recorded in this blueprint, and the
\texttt{scripts/} directory --- driven by \texttt{scripts/verify\_all.py} ---
cross-checks every finite computational theorem by algorithmically
independent recomputation in exact rational arithmetic (within this
repository; not third-party verification),
exhaustively over all cases up to a size bound, with the analytic
...
Chapter~\ref{ch:boundary}. The sources of record are public at
\url{https://github.com/SciSolv/weingarten-lean} (pinned Lean/Mathlib
toolchain, the verification suites, continuous-integration gates, and
this blueprint's \LaTeX{} source; the release version is carried by
\texttt{CITATION.cff}).
...
arXiv:0907.2719); the group-algebra invertibility root set for \emph{nonnegative}
parameter, $\{0,\dots,n-1\}$, is stated as known by Collins--Matsumoto
(arXiv:2510.21186, \S3.2), where the sign-twist symmetry
$\Wg(\sigma,-z)=(-1)^n\sgn(\sigma)\Wg(\sigma,z)$ also appears (invertible
range) --- for an \emph{unrestricted} real or complex parameter $z$ the
singular set is the full integer band $\{-(n-1),\dots,n-1\}$ (the content
spectrum $\prod_{(i,j)\in\lambda}(z+j-i)$; already at $n=2$, $z=-1$ the
element $\delta_e-\delta_{(0\,1)}$ is a zero divisor), so the
positive-only set printed there \emph{requires} the nonnegativity
qualification made explicit here (as printed, for unrestricted $z$ it
conflicts with that paper's own display~(3.2) and sign-twist symmetry); the same paper displays the rising sum rule
...
\in\salg{n}$, for $N$ in an arbitrary commutative coefficient ring $k$
(the Lean declaration is stated at this generality; the real case of the
$\ell^1$ chapter is the specialization $k=\R$).
\end{definition}
...
For every $N$ in an arbitrary commutative coefficient ring $k$ (the Lean
theorem is stated at this generality; all real, rational, and complex
uses below are specializations):
\[
...
element is singular --- with no irreducible-character theory, no Schur
functions and no spectral theory (the two one-dimensional characters,
augmentation and sign, are of course exactly what the sum rules
evaluate). The stable-range rules of Chapter~\ref{ch:wg} become
...
of the Gram data; P.~Zinn-Justin (arXiv:0907.2719) records the
convention in this two-equation form with a symmetry qualifier (there,
$\Wg$ is the \emph{symmetric} matrix satisfying the two equations), and
the bare two-equation form appears in arXiv:0906.4694 (before its
Theorem~2.1). The two-line character
...
\begin{definition}[All-ones element]\label{def:ones}
\lean{Weingarten.ones}
\leanok
\uses{def:symAlg}
$s_n \coloneqq \sum_{\sigma\in S_n}\delta_\sigma\in\salg{n}$ --- the
augmentation twin of Definition~\ref{def:alt}.
\end{definition}

\begin{lemma}[Ones absorption]\label{lem:ones_mul}
\lean{Weingarten.ones_mul}
\leanok
\uses{def:ones, lem:hom_coeffsum}
$s_n\,x = \varepsilon(x)\,s_n$ for every $x\in\salg{n}$: right
multiplication by a basis element only permutes the summands.
\end{lemma}
\begin{proof}
\leanok
\uses{def:ones}
Mirror of Lemma~\ref{lem:alt_mul} without signs: reindex by right
translation, then extend by linearity.
\end{proof}
...
$\mathcal G_n(N)$ is not a unit. The statement is known: for
\emph{nonnegative} parameter, invertibility of $\mathcal G_n(N)$ in the
group algebra holds exactly off the integer root set $\{0,1,\dots,n-1\}$
(stated as known by Collins--Matsumoto, arXiv:2510.21186, \S3.2). For an
\emph{unrestricted} real or complex parameter $z$ the singular set is the
full integer band $\{-(n-1),\dots,n-1\}$: the central character of
$\mathcal G_n(z)$ on the irreducible indexed by $\lambda\vdash n$ is the
content product $\prod_{(i,j)\in\lambda}(z+j-i)$, whose roots sweep the
band (already at $n=2$, $z=-1$ the element
$\mathcal G_2(-1)=\delta_e-\delta_{(0\,1)}$ is a zero divisor); the
positive-only set printed in arXiv:2510.21186 therefore \emph{requires}
the nonnegativity qualification adopted here --- as printed, for
unrestricted $z$ it conflicts with the paper's own display~(3.2) and
sign-twist symmetry, which confirm the band. The formalized content of this node
is the singular direction for natural $1\le N<n$; the negative-integer
companion is Theorem~\ref{thm:gram_singular_neg}, and the invertible
direction off the band (a character-spectrum fact) is \emph{not}
formalized in this repository --- the Lean development proves
invertibility only on the real range $N>n-1$
(Definition~\ref{def:wgElement}). The alternating-element proof below is
the part we have not located in print.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:alt_mul, lem:gram_eval}
...
\begin{theorem}[Gram singularity at negative integers]\label{thm:gram_singular_neg}
\lean{Weingarten.not_isUnit_gram_neg}
\leanok
\uses{def:ones, lem:ones_mul, lem:gram_eval}
For integers $0\le m\le n-1$ the Gram element at $N=-m$ is not a unit:
$s_n\,\mathcal G_n(-m) = \bigl(\textstyle\prod_{j<n}(-m+j)\bigr)s_n = 0$
while $s_n\ne 0$ in characteristic zero. With
Theorem~\ref{thm:gram_singular} this exhibits the full integer singular
band $\{-(n-1),\dots,n-1\}$: the alternating element detects the
positive-side roots through the signed character, the all-ones element
detects the negative-side roots through the augmentation.
\end{theorem}
\begin{proof}
\leanok
\uses{def:ones, lem:ones_mul, lem:gram_eval}
The rising factorial (Lemma~\ref{lem:gram_eval}) vanishes at $-m$ through
the factor $j=m$; $s_n$ is a zero divisor, so $\mathcal G_n(-m)$ is not
invertible. (Machine check: the $n=2$ determinant band scan in
\texttt{verify\_penrose\_eigen.py}.)
\end{proof}
...
pseudo-inverse qualifies; so does the genuine inverse when it exists. In
a general matrix algebra these two equations define a \emph{reflexive
generalized inverse} and do not pin down a unique $\mathcal W$; for the
\emph{central} Gram element they do
(Theorem~\ref{thm:gram_penrose_unique} below), so the two-equation
characterization is canonical here whenever it is inhabited.
...
\begin{lemma}[Centrality of the Gram element]\label{lem:gram_central}
\lean{Weingarten.gram_central}
\leanok
\uses{def:gram}
$\mathcal G_n(N)$ commutes with every element of the group algebra:
$N^{c(\sigma)}$ is a class function, so both products against a basis
element reindex to the same sum.
\end{lemma}
\begin{proof}
\leanok
\uses{def:gram}
$c(\rho g^{-1}) = c(g^{-1}\rho)$ by conjugation invariance of the cycle
count; extend by linearity.
\end{proof}

\begin{lemma}[Group-inverse uniqueness]\label{lem:penrose_comm_unique}
\lean{Weingarten.penrose_partner_unique}
\leanok
\uses{def:penrose_pair}
In any monoid, two Penrose partners of $g$ that both commute with $g$
coincide.
\end{lemma}
\begin{proof}
\leanok
$w_1 = w_1^2 g$ and $w_2 = g w_2^2$ from the second Penrose equations
plus commutation; inserting $g = g w_2 g$ and $g = g w_1 g$ gives
$w_1 = w_1 w_2 g = w_2$. Associativity only.
\end{proof}

\begin{theorem}[Canonicity of the Gram's Penrose partner]\label{thm:gram_penrose_unique}
\lean{Weingarten.gram_penrose_partner_unique}
\leanok
\uses{def:penrose_pair, lem:penrose_comm_unique, lem:gram_central}
Any two Penrose partners of $\mathcal G_n(N)$ coincide: the commutation
hypothesis is free by centrality (Lemma~\ref{lem:gram_central}), so the
two-equation characterization of the below-threshold Weingarten data is
canonical whenever it is inhabited.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:penrose_comm_unique, lem:gram_central}
Immediate from Lemmas~\ref{lem:penrose_comm_unique}
and~\ref{lem:gram_central}.
\end{proof}
...
\begin{lemma}[Per-eigenvalue Penrose rule]\label{lem:penrose_eigen}
\lean{Weingarten.IsCommPenrosePartner.smul_vecMul_eq_of_vecMul_eq_smul}
\leanok
\uses{def:comm_penrose}
Let $W$ be a commuting Penrose partner of $G$ over a commutative ring,
and let $y^{\mathsf T}G = \lambda\,y^{\mathsf T}$ with $\lambda$ a unit.
Then $\lambda\,(y^{\mathsf T}W) = y^{\mathsf T}$. No invertibility of $G$
is assumed: only the single eigenvalue must be a unit.
\end{lemma}
\begin{proof}
\leanok
\uses{def:comm_penrose}
Multiplying the eigen-row into $GWG=G$ gives
$\lambda\,(y^{\mathsf T}WG) = \lambda\,y^{\mathsf T}$; cancel the unit
$\lambda$, then $WG=GW$ turns $y^{\mathsf T}GW$ back into
$\lambda\,y^{\mathsf T}W$. Division-free: the conclusion is in
$\lambda$-multiplied form.
\end{proof}
...
In particular, over the \emph{nonnegative} integers the invertibility
threshold is $N\ge n$: singularity at $0\le N<n$ is this node's
machine-checked content, while invertibility at integer $N\ge n$ is the
known spectral endpoint (the zonal spectrum cited in this chapter's
preamble) and is \emph{not} formalized here. The nonnegativity scoping
matters: at negative integers the zonal factors
$\prod_{(i,j)\in\lambda}(N+2j-i-1)$ can be nonzero (already
$G^{\mathrm O}_2(-1)$ has determinant $-4$ and is invertible over
$\mathbb Q$), so no unqualified integer threshold statement holds.
\end{theorem}
...
\begin{theorem}[Per-eigenvalue even-rising rule]\label{thm:orth_rising_rule_eigen}
\lean{Weingarten.evenRising_smul_vecMul_one_of_partner}
\leanok
\uses{lem:penrose_eigen, thm:orth_ones_absorb}
If $W$ is a commuting Penrose partner of $G^{\mathrm O}_n(N)$ and
$r_n(N)$ is a unit, then
$r_n(N)\,\mathbf 1^{\mathsf T}W = \mathbf 1^{\mathsf T}$ --- even where
the Gram itself is singular (e.g.\ $n=2$, $N=1$). This strengthens
Theorem~\ref{thm:orth_rising_rule}: full invertibility of the Gram is
replaced by the single relevant eigenvalue.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:penrose_eigen, thm:orth_ones_absorb}
Lemma~\ref{lem:penrose_eigen} with the eigen-row
$\mathbf 1^{\mathsf T}G = r_n(N)\,\mathbf 1^{\mathsf T}$
(Theorem~\ref{thm:orth_ones_absorb} plus symmetry). (Machine check:
\texttt{verify\_penrose\_eigen.py} at the singular cell $n=2$, $N=1$.)
\end{proof}

\begin{theorem}[Per-eigenvalue falling rule]\label{thm:orth_falling_rule_eigen}
\lean{Weingarten.fallingFact_smul_vecMul_detVec_of_partner}
\leanok
\uses{lem:penrose_eigen, thm:orth_det_absorb}
Under the same partner hypothesis, if $f_n(N)$ is a unit then
$f_n(N)\,v^{\mathsf T}W = v^{\mathsf T}$ --- complementary to the exact
vanishing of Theorem~\ref{thm:orth_det_vanish} when $f_n(N)=0$.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:penrose_eigen, thm:orth_det_absorb}
Lemma~\ref{lem:penrose_eigen} with the eigen-row of
Theorem~\ref{thm:orth_det_absorb}.
\end{proof}

\begin{remark}[Integral reading]\label{rem:orth_integral}
\uses{thm:orth_det_vanish}
For Haar $O\in\mathrm O(N)$ with $N\ge n$ and $A$ the top-left
$n\times n$ corner,
$E[(\det A)^2] = n!\,\bigl(v^{\mathsf T}\mathrm{Wg}\bigr)_{q_{\mathrm{id}}}
= n!/f_n(N)$ above threshold. Below threshold ($N<n$) no $n\times n$
corner of an $N\times N$ matrix exists; the surviving object is the
$n\times n$ array $A_{ab}:=O_{\iota(a)\iota(b)}$ built from index maps
$\iota:[n]\to[N]$ --- any such array has a repeated row, so
$\bigwedge^n\mathbb R^N=0$ forces $\det A=0$ pointwise --- and
Theorem~\ref{thm:orth_det_vanish} is exactly the Weingarten shadow of that
rank deficiency (the same nonexistence-of-the-corner convention as
Remark~\ref{rem:symp_integral}). The \emph{orthogonal} Haar moment formula itself remains out
...
For natural numbers $M<n$ the symplectic Gram is not invertible (over any
nontrivial ring). In particular, over the \emph{nonnegative} integers the
invertibility threshold is $M\ge n$: singularity at $0\le M<n$ is this
node's machine-checked content, while invertibility at integer $M\ge n$
is the known spectral endpoint (via the twisted spectrum cited in this
chapter's preamble) and is \emph{not} formalized here. As in
Theorem~\ref{thm:orth_not_unit}, the nonnegativity scoping is essential
--- the closed form at negative $M$ reduces to the orthogonal Gram at
$-2M>0$, which is generically invertible.
\end{theorem}
...
\begin{theorem}[Per-eigenvalue even-falling rule]\label{thm:symp_even_falling_rule_eigen}
\lean{Weingarten.evenFalling_smul_vecMul_epsVec_of_partner}
\leanok
\uses{lem:penrose_eigen, thm:symp_eps_ones_absorb}
If $W$ is a commuting Penrose partner of the symplectic Gram and the
even-falling factorial $e_n(M)$ is a unit, then
$e_n(M)\,\varepsilon^{\mathsf T}W = \varepsilon^{\mathsf T}$ --- the
per-eigenvalue strengthening of
Theorem~\ref{thm:symp_even_falling_rule}, with no invertibility of the
Gram assumed.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:penrose_eigen, thm:symp_eps_ones_absorb}
Lemma~\ref{lem:penrose_eigen} with
Theorem~\ref{thm:symp_eps_ones_absorb}.
\end{proof}

\begin{theorem}[Per-eigenvalue shifted-rising rule]\label{thm:symp_rising_shift_rule_eigen}
\lean{Weingarten.risingShift_smul_vecMul_epsDetVec_of_partner}
\leanok
\uses{lem:penrose_eigen, thm:symp_eps_det_absorb}
If the shifted rising factorial $s_n(M)=2M(2M+1)\cdots(2M+n-1)$ is a
unit --- for $M\ge 1$ over $\mathbb Q$ this holds at \emph{every} cell,
including the whole singular band $M<n$ --- then
$s_n(M)\,(\varepsilon\odot v)^{\mathsf T}W
= (\varepsilon\odot v)^{\mathsf T}$: the per-eigenvalue strengthening of
Theorem~\ref{thm:symp_rising_shift_rule}.
\end{theorem}
\begin{proof}
\leanok
\uses{lem:penrose_eigen, thm:symp_eps_det_absorb}
Lemma~\ref{lem:penrose_eigen} with
Theorem~\ref{thm:symp_eps_det_absorb}.
\end{proof}
...
consistency statement, exactly parallel to $E[(\det)^2]$. The displayed
first moment is a project-side derivation (recorded in the design notes),
conditional on the standard $\mathrm{Sp}(M)$ Haar--Weingarten moment
formula --- itself outside the formalized scope --- exact-checked at the
cells below and \emph{not} formalized. Verified at ten
...
\section{The abstract Haar expectation}
...
(cf.\ Banica--Collins, Def.~4.1, whose Haar functional is additionally
\emph{positive}; positivity is deliberately omitted from this
interface, so every Haar state in their sense restricts to a Haar
expectation, and integration against the constructed Haar probability
measure of Section~\ref{sec:haar_construction} provides a positive
instance --- the moment theorems below consume only linearity,
normalization, and bi-invariance). The domain restriction to
\emph{continuous} observables is load-bearing: for $N\ge1$ the same
interface on all functions $\mathrm U(N)\to\mathbb C$ is uninhabited
(a coboundary/orbit argument, recorded as a documented argument with
the definition, \emph{not} formalized); at $N=0$ the group is a single
point and evaluation inhabits even the all-functions interface
(\texttt{haarExpectationZero}).
\end{definition}
...
\]
equivalently the truncated quotient is $\beta/2\pmod{\beta^{5}}$
--- exact finite-$N$ mock-Gaussianity (at $N=1$, $w_{3}=-1/16$).
The companion expansion gives truncated quotient
$1+\beta^{2}/4\pmod{\beta^{4}}$. (The analytic readings
$\langle\operatorname{Tr}U\rangle_{\beta}=\beta/2+O(\beta^{5})$ and
$\langle|\operatorname{Tr}U|^{2}\rangle_{\beta}=1+\beta^{2}/4
+O(\beta^{4})$ are prose glosses of these formalized coefficient
identities: the machine-checked content is the finite truncated
quotient, and the Taylor-remainder bridge to the actual integral
quotient is \emph{not} formalized here.)
\end{theorem}
\begin{proof}
\leanok
\uses{thm:trace_words}
...
\Wg\bigl(\swap{0}{1},N\bigr)=-\frac1{N(N^{2}-1)}
\]
--- matching the fourth-moment table of
Theorem~\ref{thm:fourth_moments} (the transposition is written in the
standing $0$-indexed convention; textbook sources print it as
$(1\,2)$).
\end{theorem}
\begin{proof}
\leanok
...
$2\Wg(e)+2\Wg(\swap{0}{1})=2/(N(N+1))$, sign consistency with the parity
...
\item T.~Banica and B.~Collins, \emph{Integration over compact quantum groups}, arXiv:math/0511253. (Def.~4.1: the \emph{positive} bi-invariant Haar functional; the expectation interface of Chapter~\ref{ch:haar} keeps its linearity, normalization, and bi-invariance and deliberately omits the positivity --- every Haar state in their sense restricts to an instance.)
